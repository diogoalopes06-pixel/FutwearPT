...
function getEffectiveStatus(status, paymentStatus) {
  return paymentStatus === "paid" && status === "pending" ? "confirmed" : status;
}

function Timeline({ status, paymentStatus }) {
  const effective = getEffectiveStatus(status, paymentStatus);
  ...
  return <div className="grid grid-cols-5 gap-2 mt-2 min-w-[620px]">{STAGES.map((s, i) => { const Icon = s.icon; const active = i <= idx; return <div key={s.key} className="flex flex-col items-center text-center"><div className={`w-10 h-10 grid place-items-center rounded-full border-2 ${active ? "border-brand-red bg-brand-red text-white" : "border-brand-border bg-white text-brand-muted"}`}><Icon size={16} /></div><p className={`mt-2 text-[10px] uppercase tracking-[0.12em] ${active ? "text-brand-espresso" : "text-brand-muted"}`}>{s.label}</p></div>; })}</div>;
}
